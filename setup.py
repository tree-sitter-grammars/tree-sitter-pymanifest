...
sources = [
    "bindings/python/tree_sitter_pymanifest/binding.c",
    "src/parser.c",
]
...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_pymanifest", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_pymanifest": ["*.pyi", "py.typed"],
        "tree_sitter_pymanifest.queries": ["*.scm"],
    },
    ext_package="tree_sitter_pymanifest",
    ext_modules=[
        Extension(
            name="_binding",
            sources=sources,
